...
void loop() {
  ...
  if (wav->isRunning()) {
    ...
    // Si la reproducción ha finalizado, esperar 2 segundos y reiniciar
    ...
    // Reiniciar la reproducción del mismo archivo
    reproducirAudio();
  }
}
*/
...
void handleRecord();
void handlePlay();
void handleRecordPlayWithDelay();
...
void setup() {
  ...
  server.on("/", []() {
    server.send(200, "text/html", R"(
      ...
      <body>
        ...
        <button onclick="location.href='/recordplaydelay'">Grabar y Reproducir con Delay</button>
      </body>
      </html>
    )");
  });
  ...
  server.on("/recordplaydelay", handleRecordPlayWithDelay);
...
void handleRecord() {
  ...
  server.send(200, "text/plain", "Grabación completada");
}
...
void handleRecordPlayWithDelay() {
  // Grabar audio por 10 segundos y luego reproducirlo con un delay de 2 segundos
  handleRecord();
  delay(2000);
  handlePlay();
}
